...
> HanaAgent 侧栏插件：把提示词、常用文本存成 手边的markdown文件，从此再也不用去找电脑上的提示词库，双击直接复制到剪贴板，且能让Agent一键整理。自定义提示词仓库路径，可以直接对接旧提示词仓库。

## 特性

- **提示词库**：一个词条一个 `.md` 文件，可进 Git、可备份
- **胶囊卡片**：单击展开预览，**双击复制正文**（不带 frontmatter）
- **右键菜单**：词条（编辑 / 复制 / 发送给 Agent / 打开 / 复制路径 / 删除 / 重命名）；目录（新建 / 重命名 / 删除）
- **拖动排序**：目录与词条顺序可调，重启后保持
- **Agent 工具**：6 个工具，可以让 Agent 直接搜索、读取、写入提示词
