...
export class ShelfStore {
  ...
  /** 强制重新扫描磁盘并重建内存（外部改文件 / Agent 写入后手动同步用） */
  async rescan() {
    await this.load();
   ...
   * 备份用 read + write（宿主资源层 rename 对已存在目标/新目标均可能失败，
   * 不依赖 rename）；degraded 只在「备份也失败」时置位。 */
  async writeFileAtomic(rel, content) {
    ...
    let backed = true;
    try {
      const old = await this.tryRead(rel);
      if (old != null) await this.ctx.resources.write(this.ref(bak), old);
    } catch {
      backed = false;
    }
    await this.ctx.resources.write(this.ref(rel), content);
    await this.tryDelete(tmp);
    if (!backed && this.state) this.state.degraded = '写入降级：旧文件备份失败（read/write 均不可用）';
  }
  ...
  async load() {
    ...
    const disk = await scanDirectoryTree(this.dataDir, this.fsLike());    let index, source, warning;
    let cleanInit = false;
  ...
  async createPrompt(dir, name, content) {
    ...
    // 新建卡片插入目录顶部（最新在上），索引顺序同步
    dirEntry.entries.unshift(entry);
  ...
  async deleteDir(name) {
    ...
    // 递归删除：先删目录下全部词条文件，再删目录（用户已确认，不再要求先清空）
    for (const entry of dirEntry.entries) {
      try { await this.ctx.resources.delete(this.ref(entryRel(dirEntry.name, entry.filename))); } catch {}
    }
