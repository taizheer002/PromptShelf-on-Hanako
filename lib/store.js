...
/** 生成点前缀临时名（.basename.tmp），使扫描器自动忽略残留 tmp；basename 已点开头则不再重复加点 */
function dotTmpName(rel) {
...
export class ShelfStore {
  ...
  async writeFileAtomic(rel, content) {
    ...
    try {
      ...
      if (this.state) this.state.degraded = '资源层不支持原子 rename，已回退为备份+直写';
    }
  }
  ...
  findEntry(relPath) {
    ...
    return this.getEntry(dir, filename);
  }
  ...
  async createPrompt(dir, name, content) {
    ...
    if (!isValidPathInput(dir) || !isValidPathInput(name)) return { ok: false, error: '非法路径' };
    ...
    const title = defaultTitleFromContent(content);
  ...
  async updatePrompt(relPath, content) {
    if (!this.state) return { ok: false, error: '尚未初始化（dataDir 未配置）' };
    if (!isValidPathInput(relPath)) return { ok: false, error: '非法路径' };
    const { dir, filename } = this.splitRelPath(relPath);
  ...
  async renamePrompt(relPath, newName) {
    if (!this.state) return { ok: false, error: '尚未初始化（dataDir 未配置）' };
    if (!isValidPathInput(relPath) || !isValidPathInput(newName)) return { ok: false, error: '非法路径' };
    const { dir, filename } = this.splitRelPath(relPath);
    const dirEntry = this.state.directories.find((d) => d.name === dir);
    const entry = this.getEntry(dir, filename);
  ...
  async movePrompt(relPath, targetDir) {
    if (!this.state) return { ok: false, error: '尚未初始化（dataDir 未配置）' };
    if (!isValidPathInput(relPath) || !isValidPathInput(targetDir)) return { ok: false, error: '非法路径' };
    const { dir, filename } = this.splitRelPath(relPath);
  ...
  async createDir(name) {
    if (!this.state) return { ok: false, error: '尚未初始化（dataDir 未配置）' };
    if (!isValidPathInput(name)) return { ok: false, error: '非法路径' };
  ...
  async renameDir(name, newName) {
    if (!this.state) return { ok: false, error: '尚未初始化（dataDir 未配置）' };
    if (!isValidPathInput(name) || !isValidPathInput(newName)) return { ok: false, error: '非法路径' };
