...
/**
 * 合并磁盘扫描结果与既有索引：
 * - 目录主顺序 = 索引顺序（用户拖动/创建的顺序），磁盘新增目录按磁盘序补尾；
 * - 词条同理：索引 items 在前，磁盘新增按磁盘序补尾；
 * - 索引中磁盘已不存在的目录项与词条剔除。返回新 OrderIndex（不改入参）。
 * 这样无论磁盘 mtime 怎么变，用户手动排过的顺序在重启后保持不变（与卡片同逻辑）。
 */
export function mergeIntoIndex(disk, index) {
  const indexDirs = (index && Array.isArray(index.directories)) ? index.directories : [];
  const byName = new Map(indexDirs.map((d) => [d.name, d]));
  const dirNames = [
    ...indexDirs.map((d) => d.name).filter((n) => disk.dirs.includes(n)),
    ...disk.dirs.filter((n) => !byName.has(n)),
  ];
  const directories = dirNames.map((name) => {
